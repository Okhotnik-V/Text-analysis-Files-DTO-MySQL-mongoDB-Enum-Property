...
public class SelectorInput implements SelectedInput {
    ...
    public void select() {
        ...
        MongoDBReading mongoDBReading = new MongoDBReader(null);
        ...
        if (inputType.equals(Inputter.CONSOLE.getInputer())) {
            ...
            System.out.println("Select database: mongoDB or mySQL ");
            String inputDB = scanner.next();
            if (inputDB.equals("mongoDB")) {
                check = checking.determine(mongoDBReading.connectingMongo());
                resultRead = counting.identify(check, Inputter.DB.toString());
            } else if (inputDB.equals("mySQL")) {
